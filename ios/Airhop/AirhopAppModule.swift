...
final class AirhopAppModule: NSObject, RCTBridgeModule {
    ...
    @objc
    func restart(_ resolve: @escaping RCTPromiseResolveBlock,
                 rejecter reject: @escaping RCTPromiseRejectBlock) {
        reject("UNSUPPORTED", "iOS has no supported way to relaunch itself", nil)
    }

    @objc
    func setAutoStartOnBoot(_ enabled: Bool,
    ...
    @objc
    func recentLog(_ resolve: @escaping RCTPromiseResolveBlock,
                    rejecter reject: @escaping RCTPromiseRejectBlock) {
        do {
            ...
            AirhopLog.app.warning("could not read the log store: \(error.localizedDescription, privacy: .public)")
            resolve("")
        }
    }
}
