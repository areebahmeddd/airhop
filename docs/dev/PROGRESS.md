...
- [x] `src/core/mesh/routing/fragment-manager.ts`: split/reassemble, 30 s idle timeout, 128-slot concurrent cap
...
| M08  | Attachment forged, aimed at the wrong thread, or tagged into an unjoined room                                    | Refused on all three, and a forged public photo is not relayed either; an attachment carries the same rules text does                                                                                     |
...
| M07  | Recorded voice burst played out of a stranger's phone later                                                      | Refused on freshness and not relayed, even to a phone that never heard the original; a forged burst padded past a frame and sent as fragments plays nothing. A valid signature does not make a burst live |
...
| S15  | 1100 forged messages under a real peer's ID, signed by another key, then a latecomer arrives                     | The forgeries are neither kept nor served; the latecomer's backfill carries the real peer's messages                                                                                                      |
| S16  | History a newcomer cannot verify, offered by every neighbour it asks                                             | Offered once, then advertised as held and not offered again each round                                                                                                                                    |
| X04  | Gossip sync with a bitchat-ios phone right after first contact (control)                                         | Crosses both ways in bitchat-ios's two rounds: a three-hour-old message and a day-old fragmented board post are taken, so S11 to S16 refuse nothing bitchat sends                                         |
...
| M14  | First fragments of a file under a stranger's ID, of a sealed file with no session, and 20 public streams at once | No card for either forgery, at most three receiving cards, and public ones unnamed; the streams still land                                                                                                |
...
| R08  | Forged board post, photo and voice frame under a peer's name, beside a relay                                     | Relayed by no one and shown by no one; the genuine notice, photo and voice still cross the chain                                                                                                          |
| R09  | DM addressed to the relay in the middle of a chain                                                               | That phone forwards nothing addressed to it                                                                                                                                                               |
| R10  | A twin running this phone's identity                                                                             | This phone relays none of the twin's announces, and still warns that the identity is running elsewhere                                                                                                    |
...
| 87  | Files, board posts and voice frames relayed before they were checked                                                                 | Medium   | Fixed - checked before the relay and before dedup, with LEAVE, as bitchat-ios; reassembled packets meet the same gate                                                               |
| 88  | Gossip stored packets before verifying them, in one bucket every kind shared                                                         | Low      | Fixed - accepted packets only, one store per kind with bitchat-ios's sizes; a refused reply is advertised, never served                                                             |
| 89  | One sync filter over every type let busy messages push board posts behind the cursor                                                 | Low      | Fixed - bitchat-ios's rounds: announces and messages every 15 s, board posts alone every 60 s                                                                                       |
| 90  | Fragment reassembly accepted empty and tiny non-final fragments                                                                      | Info     | Fixed - empty ones refused, and non-final ones under 64 bytes, as bitchat-ios cuts them                                                                                             |
| 91  | Receiving cards appeared for anyone, uncapped, under a claimed name                                                                  | Low      | Fixed - only for a sender whose key is held (and a session, for a sealed file), at most three, unnamed in public rooms                                                              |
| 92  | Relay targeting and TTL did not follow bitchat-ios, so a node forwarded packets addressed to it and relayed its twin's               | Low      | Fixed - `relayDecision` transcribes `RelayController`: degree clamps, directed traffic at full depth, nothing for or from this node                                                 |
| 93  | Origin TTL drawn from 5 to 7 whatever the relays around it clamp to                                                                  | Low      | Fixed - drawn from `[ceiling-2, ceiling]` of the same relay limit at this phone's degree                                                                                            |
| 94  | Owed read receipts grew until the thread was opened                                                                                  | Low      | Fixed - bounded per conversation, and dropped with a deleted or blocked thread                                                                                                      |
| 95  | Maps that never shrank: a closed link's pong budget, idle sync limiter keys, stale sync requests                                     | Low      | Fixed - each pruned on link close or on the sync tick                                                                                                                               |
| 96  | GCS request decode accepted `m = 0` and `p` outside 1..32                                                                            | Low      | Fixed - refused before any response budget is spent, as bitchat-ios                                                                                                                 |
