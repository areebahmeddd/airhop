...
- Android recognises a peer across a restart of its session. A WiFi toggle or re-attach on the far side hands out a fresh `PeerHandle` for the same device, and dialling it again requests a second data path to a device already holding one, which the framework answers by replacing the first: the live socket dies and the new connect aborts. An 8-byte per-process instance id beside the tiebreak token, repeated after the connect request byte, is what lets the initiator skip a linked peer and the responder replace a link its peer has evidently lost. bitchat/android keys the same guard on the peer ID
- Android restarts its own discovery. A publish/subscribe pair can go quiet under the framework with no callback saying so, and the cure is what a WiFi toggle does: close the session and attach again. The module does that itself after five minutes with nobody found and no link held, through the same teardown-then-report path a framework termination takes, so the reconciler sees one shape of event. bitchat/android refreshes on the same rule
